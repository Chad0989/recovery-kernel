/* linux/arch/arm/mach-msm/board-sonywvga-panel.c
 *
...
static struct platform_driver sonywvga_driver = {
	...
	.driver = { .name = "sonywvga-panel" },
};

static int __init sonywvga_init(void)
{
	return platform_driver_register(&sonywvga_driver);
}

static int __init sonywvga_backlight_init(void)
{
	return platform_driver_register(&sonywvga_backlight_driver);
}

device_initcall(sonywvga_init);
module_init(sonywvga_backlight_init);
